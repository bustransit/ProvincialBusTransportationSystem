 ...
 * @author Admin
 */
public class SuccessionPlanController extends Application implements Initializable {
...
//        ModuleFunctions f = new ModuleFunctions();
        //f.loadFunctions("hr");
        Parent root = FXMLLoader.load(getClass().getResource("SuccessionPlan.fxml"));
    ...
    public void populateSuccesionPlanePane(){
        ...
        try {
            ...
            Logger.getLogger(SuccessionPlanController.class.getName())
                  .log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(SuccessionPlanController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    @FXML
    private void searchSuccession(KeyEvent event) {
        String s = ((CustomTextField) event.getSource()).getText();
        String q = "SELECT * FROM succession_plan, position WHERE position.description LIKE '%"+s+"%'";
        rs = db.displayRecords(q);
        fpSuccessionPlan.getChildren().clear();
        try {
            while(rs.next()){
    ...
    private void toPDF(String q, String reportName, String module) {
        try {          
            ...
            try {
                ...
                Logger.getLogger(SuccessionPlanController
